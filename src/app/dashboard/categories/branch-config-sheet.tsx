...
export function BranchConfigSheet({
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-5xl w-full p-0 flex flex-col overflow-hidden bg-muted/10">
        ...
        <div className="flex items-center justify-between p-4 border-b bg-background sticky top-0 z-20">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button className="gap-2 bg-primary hover:bg-primary/90 text-primary-foreground" onClick={handleSave}>
        ...
        <div className="flex-1 overflow-y-auto">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="w-full justify-start rounded-none border-b bg-background px-6 h-12 sticky top-0 z-10">
              <TabsTrigger value="basic" className="data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none h-full gap-2 text-xs font-semibold uppercase tracking-wider">
                <Info className="h-4 w-4 text-primary" /> Basic Information
              </TabsTrigger>
              <TabsTrigger value="seating" className="rounded-none h-full gap-2 text-xs font-semibold uppercase tracking-wider">
                <Users className="h-4 w-4" /> Seating & Capacity
              </TabsTrigger>
              <TabsTrigger value="photos" className="rounded-none h-full gap-2 text-xs font-semibold uppercase tracking-wider">
                <ImageIcon className="h-4 w-4" /> Photos & Gallery
              </TabsTrigger>
              <TabsTrigger value="hours" className="rounded-none h-full gap-2 text-xs font-semibold uppercase tracking-wider">
                <Clock className="h-4 w-4" /> Opening Hours
              </TabsTrigger>
              <TabsTrigger value="reservations" className="rounded-none h-full gap-2 text-xs font-semibold uppercase tracking-wider">
                <CalendarCheck className="h-4 w-4" /> Reservation Settings
              </TabsTrigger>
              <TabsTrigger value="customization" className="rounded-none h-full gap-2 text-xs font-semibold uppercase tracking-wider">
                <Palette className="h-4 w-4" /> Customization
              </TabsTrigger>
            </TabsList>

            {/* Basic Information Content */}
            <TabsContent value="basic" className="p-8 space-y-10 focus-visible:ring-0">
              {/* Restaurant Details */}
              <section className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input defaultValue={branch?.name || "Bloomsbury's"} placeholder="e.g. Bella Cucina Italian" />
                  </div>
                  <div className="space-y-2">
              ...
              <section className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input defaultValue={branch?.address || ""} placeholder="e.g. 123 Main Street" />
                  </div>
                  <div className="space-y-2">
                    <Label className="font-semibold text-muted-foreground text-xs uppercase tracking-wider">City</Label>
                    <Input defaultValue={branch?.location || ""} placeholder="e.g. San Francisco" />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input placeholder="https://maps.google.com/..." className="pr-10" />
            ...
            <TabsContent value="hours" className="p-8 space-y-6 focus-visible:ring-0">
              <div className="flex items-center justify-between">
                <div>
                ...
                <Button variant="outline" size="sm">Apply to All Days</Button>
              </div>

              <div className="space-y-3">
                {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map((day) => (
                  <Card key={day} className="border shadow-none">
            ...
            {['seating', 'photos', 'reservations', 'customization'].map(tab => (
              <TabsContent key={tab} value={tab} className="p-20 text-center space-y-4 focus-visible:ring-0">
